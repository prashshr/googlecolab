...
from risk_engine import guardrails
...
INDIA_PARAMETERS = {
    ...
    "B1_BASE_AMOUNTS": {0.05: 500, 0.075: 750, 0.10: 1000},
    "MAX_NORM_CAP": 60000.0,
    "HEAVY_BASE": 1000.0,
    "MAX_HEAVY_PER_CYCLE": 3,
...
def select_b1_base_amount(drawdown, thresholds, base_amounts):
    ...
    if not eligible:
        return None, 0.0
    chosen = max(eligible)
    if chosen not in base_amounts:
        fallback_key = min(base_amounts.keys())
        chosen = fallback_key
    return chosen, base_amounts[chosen]


def attempt_trade(order, action_fn, *args, **kwargs):
    """
    Run guardrails and, if approved, execute the provided callable.
    """
    order = dict(order)
    order.setdefault("simulated", TRADING_MODE != "live")
    try:
        guardrails.before_order(order, mode=TRADING_MODE)
    except guardrails.GuardrailRejection as exc:
        print(f"{COLOR_RED}[GUARDRAIL BLOCK]{COLOR_RESET} {order.get('ticker')} {exc}")
        return None
...
class Portfolio:
    ...
    def __init__(self, ticker, params):
        ...
        # Track which B1 rungs have fired inside each ATH cycle
        self.normal_buys_by_cycle = {}

    # ---------------------------------------------
    # Normal buy (never uses low-marker pool)
    # ---------------------------------------------
    def buy(self, date, price, amount, max_cap, reason="NORMAL_BUY"):
...
def run_ticker_strategy(ticker, gold_vault, params, start_date=STRATEGY_START_DATE, existing_state=None):
    ...
    trend_flags = params.get("B_TREND_FILTER_ENABLED") or {
        f"B{i+1}": True for i in range(len(b1_thresholds))
    }
    ...
    if existing_state and existing_state.get("portfolio"):
        ...
        if not hasattr(p, "heavy_counts"):
            p.heavy_counts = {}
        if not hasattr(p, "normal_buys_by_cycle"):
            p.normal_buys_by_cycle = {}
    ...
    normal_buys_by_cycle = p.normal_buys_by_cycle
    ...
    for (date, price, ath_pre) in signals:
        ...
        # Trend filter + determine rung
        dd = (ath_pre - price) / ath_pre
        drawdown_level, base_amt = select_b1_base_amount(dd, b1_thresholds, b1_base_amounts)
        available_cap = max(0.0, max_norm_cap - p.invested)
        order_amount = min(base_amt, available_cap)
        if order_amount <= 0:
            continue

        rung_idx = None
        rung_label = None
        if drawdown_level is not None:
            for idx, thr in enumerate(b1_thresholds):
                if abs(drawdown_level - thr) < 1e-9:
                    rung_idx = idx
                    rung_label = f"B{idx + 1}"
                    break

        cycle_for_date = cycle_by_date.get(date)
        rung_hits = None
        if cycle_for_date is not None:
            rung_hits = normal_buys_by_cycle.setdefault(cycle_for_date, set())

        if rung_label and rung_hits is not None and rung_label in rung_hits:
            # already executed this rung in the current ATH cycle
            continue

        apply_trend = True
        if rung_label:
            apply_trend = trend_flags.get(rung_label, True)
        ...
        if apply_trend and not trend_ok:
            ema_val = float(ema200.loc[date]) if not np.isnan(ema200.loc[date]) else float("nan")
            ema_cap = ema_val * trend_ema_ext if not np.isnan(ema_val) else float("nan")
            rung_text = rung_label or "B?"
            print(
                f"{COLOR_YELLOW}[TREND_SKIP]{COLOR_RESET} {date.date()} {ticker} "
                f"{rung_text} dd={dd*100:.2f}% price={price:.2f} limit={ema_cap:.2f}"
            )
            continue

        normal_shares_before = sum(c.shares for c in p.cycles if not c.heavy)
        dd_suffix = None
        if drawdown_level:
            pct = drawdown_level * 100
            dd_suffix = f"{pct:.2f}".rstrip("0").rstrip(".")
        buy_reason = f"NORMAL_BUY_{dd_suffix}" if dd_suffix else "NORMAL_BUY"
        order_reason_prefix = rung_label or "B"
        order_reason = f"{order_reason_prefix}_{dd_suffix}" if dd_suffix else order_reason_prefix
        order_payload = {
            "ticker": ticker,
            "side": "BUY",
            ...
            "max_investment": max_norm_cap,
            "current_position_shares": normal_shares_before,
            "timestamp": date.to_pydatetime(),
            "simulated": TRADING_MODE != "live"
        }

        executed = attempt_trade(order_payload, p.buy, date, price, order_amount, max_norm_cap, buy_reason)
        if not executed or executed <= 0:
            continue
        if executed > 0:
            if rung_label and rung_hits is not None:
                rung_hits.add(rung_label)
    ...
    for check_date in future_dates:
        ...
        if check_price >= target:
            sell_qty = normal_shares * TP_FRAC
            order_payload = {
                "ticker": ticker,
                "side": "SELL",
                "qty": sell_qty,
                "price": float(check_price),
                "amount": sell_qty * check_price,
                "reason": f"TP_{phase}",
                "current_position_shares": normal_shares,
                "timestamp": check_date.to_pydatetime(),
                "simulated": TRADING_MODE != "live"
            }

            prof = attempt_trade(order_payload, p.sell_fraction, check_date, check_price, TP_FRAC, f"TP_{phase}")
            if prof is None:
                continue
    ...
    for d in marker_dates:
        ...
        order_payload = {
            "ticker": ticker,
            "side": "BUY",
            "qty": heavy_base / price,
            "price": float(price),
            "amount": heavy_base,
            "reason": "HEAVY_LOW_MARKER",
            "current_invested": p.invested,
            "max_investment": max_norm_cap,
            "current_position_shares": sum(c.shares for c in p.cycles if not c.heavy),
            "timestamp": d.to_pydatetime(),
            "simulated": TRADING_MODE != "live"
        }

        executed = attempt_trade(order_payload, p.heavy_buy, d, price, heavy_base)
        if not executed or executed <= 0:
            continue
        if executed > 0:
...
def _format_table_value(value):
    ...
    if isinstance(value, (np.floating, float)):
        if math.isnan(value):
            return ""
        txt = f"{value:,.2f}" if abs(value) >= 1 else f"{value:.6f}"
...
def render_ascii_table(df, columns):
    if df.empty:
    ...
    for col in columns:
        ...
        formatted = [_format_table_value(v) for v in series]
        col_values[col] = formatted
        widths[col] = max(len(col), *(_visible_len(v) for v in formatted))

    border = "+-" + "-+-".join("-" * widths[col] for col in columns) + "-+"
    header_cells = [_pad_visible(col, widths[col]) for col in columns]
    lines = [border, "| " + " | ".join(header_cells) + " |", border]

    for idx in range(len(df)):
        cells = []
        for col in columns:
            val = col_values[col][idx]
            cells.append(_pad_visible(val, widths[col]))
...
def build_ticker_report(res):
    ...
    table_df = dfp[show].copy()
    for col in show:
        table_df[col] = table_df[col].fillna("")

    print(render_ascii_table(table_df, show))


# ---------------------------------------------------------------------------
# 10. GLOBAL PORTFOLIO SUMMARY (ALL TICKERS)
# ---------------------------------------------------------------------------

def build_global_summary(all_results, gold_vault=None):
    """
    Builds:
    - Per-ticker table
    - Combined totals
    ...
    total_new_money = df["new_money_used"].sum()
    total_tp_used = df["tp_used"].sum()
    ...
    print("="*80)
    summary_cols = ["ticker", "invested", "profit_booked", "held_value",
                    "final_value", "pnl", "gold_used", "tp_used", "new_money_used"]
    print(render_ascii_table(df[summary_cols], summary_cols))
    print("-"*80)
    ...
    print(f"Total TP profits reused : {total_tp_used:,.2f}")
    print(f"Total new money invested : {total_new_money:,.2f}")

    if gold_vault is not None:
        pocket_invested = gold_vault.total_principal + total_new_money
        gold_value = gold_vault.current_value()
        cash_in_portfolio = max(0.0, total_profit - total_tp_used)
        total_current_value = total_held + cash_in_portfolio + gold_value
        if pocket_invested > 1e-9:
            pocket_gain_pct = ((total_current_value - pocket_invested) / pocket_invested) * 100.0
        else:
            pocket_gain_pct = float("nan")
        print("="*80)
        print("Pocket capital summary (personal money only)")
        print(f"Total money invested (gold deposits + direct cash) : {pocket_invested:,.2f}")
        print(f"Total current value (stocks + cash + gold)       : {total_current_value:,.2f}")
        if math.isnan(pocket_gain_pct):
            print("Pocket capital gain : n/a")
        else:
            print(f"Pocket capital gain : {pocket_gain_pct:,.2f}%")
        print("="*80)
...
# ---------------------------------------------------------------------------
# 11. GOLD VAULT SUMMARY (PRINCIPAL-ONLY, LIFO WITH DATE FILTER)
# ---------------------------------------------------------------------------
...
def main():
    ...
    # GLOBAL summaries
    build_global_summary(all_results, gold_vault)
...
# ============================================================================
# STRATEGY B1.2 — CURRENT CONFIGURATION SNAPSHOT
# ============================================================================
# • Watchlist: NVDA, MSFT, PLTR, TSLA, AMZN, GOOG, META, AAPL.
# • Normal buy ladder (B1) fires when price drops 15% / 20% / 25% from ATH.
#     - Order sizes per rung: $500 / $750 / $1,000.
#     - Each rung optionally enforces the trend filter; defaults: B1/B2/B3 = ON.
#     - Trend filter: allow buys if drawdown ≥ 25%, or ≥ 15% with price ≤ EMA200*1.30.
#     - Guardrails: max 8,500 USD invested per ticker (normal + heavy), and 500 shares max.
# • Heavy low-marker buys:
#     - Conditions: drawdown ≥ 30%, price ≤ EMA200*0.85, volume ≥ 1.3×VOL60,
#       logATR ≥ 1.3×ATR60, CLV ≥ 0.40, RSI(14) ≤ 40.
#     - Order size: fixed 2,000 USD, up to 3 heavy buys per ATH cycle.
#     - Heavy cycles are never sold; they still increase invested capital.
# • Funding order for any BUY: withdraw gold principal (LIFO, date-validated),
#   then TP-profit pool, then new cash. Each trade log records the split.
# • Gold vault overlay:
#     - Ticker 4GLD.DE, deposits $1,000 on the first valid trading day of each month.
#     - Withdrawals may only use principal, never gold profits.
#     - Remaining gold position (principal + profit) stays invested and is reported.
# • Take-profit state machine per ticker:
#     - New ATH resets TP phases to TP1.
#     - Targets: ATH×1.20, ×1.40, ×1.60 (TP1/2/3 respectively).
#     - Each target sells 15% of remaining normal shares; heavy shares ignored.
#     - Realized gains flow into the low-marker pool for future heavy buys.
# • Risk guardrails executed before every order: position/share caps, investment caps,
#   price freshness, duplicate-order prevention, session/broker health, etc.
# • Reporting:
#     - Per-ticker trade log (ATH_EVENT, BUY, SELL, HEAVY_BUY) with funding sources.
#     - Global summary with invested/profit/held metrics and XIRR.
#     - Pocket-capital view: compares personal cash deployed (gold deposits + new money)
#       versus current value (stocks + cash + gold) and prints gain %.
#     - Gold vault summary plus CSV/XLSX exports for summary/trade/state snapshots.
# ============================================================================
